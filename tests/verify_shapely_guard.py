"""Verify Shapely-based intra-tree crossing guard in optimize_multi_tree."""
import os, sys, math, copy, random as _rnd, threading, time
...
def intra_per_tree(trees):
    return [st.count_crossings([t])[1] for t in trees]
...
# ── Production optimizer run (for timing + cost + crossing counts) ────────────
print()
print("Running optimize_multi_tree for 2000 iterations (production code)...")
MAX_IT   = 2000
results  = {}
...
print(f"  Per iter  : {1000*wall_time/MAX_IT:.1f}ms")
print(f"  Under 5 min: {'YES' if wall_time < 300 else 'NO -- too slow'}")

# ── Instrumented re-run for revert counts (seed=42) ──────────────────────────
print()
print("=" * 60)
print("Intra-crossing revert count (instrumented, seed=42)")
print("=" * 60)
    ...
    if delta < 0 or rng.random() < math.exp(-delta / max(T, 1e-12)):
        ...
        for ai in incident_idx:
            if len(tree.edge_polylines[ai]) < 2:
                continue
            line_a      = LineString(tree.edge_polylines[ai])
            orig_line_a = LineString(orig_edges[ai])
            for bi in non_incident_idx:
                line_b = LineString(tree.edge_polylines[bi])
                try:
                    if line_a.crosses(line_b) and not orig_line_a.crosses(line_b):
...
pct_intra = 100 * n_intra_rev / max(n_metropolis, 1)
pct_disp  = 100 * n_disp_rev  / max(n_metropolis, 1)
print(f"  Metropolis-accepted moves           : {n_metropolis}")
print(f"  Reverted by Shapely intra guard     : {n_intra_rev}  ({pct_intra:.1f}%)")
print(f"  Reverted by displacement guard      : {n_disp_rev}  ({pct_disp:.1f}%)")
print(f"  Committed moves                     : {n_metropolis - n_intra_rev - n_disp_rev}")
